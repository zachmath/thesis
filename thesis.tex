...
\usepackage[sort&compress,numbers]{natbib} % For some reason, this blocks the Bibliography from appearing in the table of contents and doesn't create the required bibliography title page. I've manually added those things back later in the document, but it's a messy fix. On the other hand, I could get rid of this line but then my bibliography uses authors throughout the text instead of numbers. There might be a better way to do that instead, but whatever. I've got something that works so I'm calling it good.
\usepackage{doi}
\usepackage{hyperref}
...
\tocentry{BIBLIOGRAPHY}{Bibliography}% Pretty hacky way of getting this to actually show up in the Table of Contents, but it seems to work
...
%% preamble and here.
\clearpage\thispagestyle{plain}\vspace*{\fill} % More hacky workaround...
\begin{center}
